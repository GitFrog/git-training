...
+ If **blue** has not deviated since **green** branched off, a fast-forward merge
is attempted
...
+ 3-way merge attempted. Git checks whether changes affect the same files
...
+ Changes affect the same files. Git prompts for resolution, adds conflict markers.
...
+ **Note:** If a file has been changed in only one of the branches, then git will not consider it a conflict, and changes will be fast-forwarded. 
...
5. Example 3: pseudo conflict (might expect a conflict, but it's really not - pointers get updated)
   ...
   + Why? Because updates was "1-way"; ONLY the new-name branch was really updated _in time_. Show timestamps
...
---
# Hooray ! You made it through Part 1!
...
+ To bring remote changes locally:
  ...
  + `git fetch`: transfers from remote to your machine, but does not attemp reconciliation.
